...
class Lambdasian {
  constructor(objAttributes) {
    ...
    this.age = objAttributes.age;
    this.location = objAttributes.location;
  }
  speak() {
...
class Instructor extends Lambdasian {
  ...
  assignGrade(student) {
    let maxPlus = Math.random() * (100 - student.grade);
    let maxMinus = Math.random() * student.grade;
    let randomChance = Math.floor(Math.random() * 2);
    if (randomChance === 0) {
      student.grade -= maxMinus;
    } else {
      student.grade += maxPlus;
    }
...
class Student extends Lambdasian {
  ...
  graduate() {
    if (this.grade >= 70) {
      return `Congratulations! You have graduated with a score of ${this.grade}.`
    } else {
      return `Not there Yet! But keep at it and we will be celebrating your graduation soon.`
    }
  }
